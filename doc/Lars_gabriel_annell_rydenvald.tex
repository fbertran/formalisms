...
Dû à une limitation dans la librairie \lstinline|C3| qui est une dépendance de \lstinline|Layeredata|, la vérification de propriétés sur les opérateurs ne fonctionne pas. En utilisant la construction d'opérateurs et d'expressions des sections \lstinline|2.3| et \lstinline|2.4|, on peut écrire
\begin{minted}{lua}
addition = {
	[refines] = { addition_expression },
	operator  = addition_expression[meta][expression].addition_operator,
...
\end{minted}
et c'est accepté par \lstinline|Layeredata| alors qu'on devrait avoir une erreur sur l'expression \lstinline|addition| lorsqu'on lui passe une chaîne de caractères comme opérande. \newline
Le problème identifié est que \lstinline|addition| hérite du modèle abstrait \lstinline|addition_expression|, or il devrait référencer une instance de \lstinline|addition_expression|. Cependant, en corrigeant cela, la librairie \lstinline|C3| s'arrête sur une erreur.
...
\subsection{Fonctions de capture notables}
