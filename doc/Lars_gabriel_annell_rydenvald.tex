...
\begin{enumerate}
	...
	\item Pour chaque type d'opérateur, on crée le pattern correspondant. Par exemple, l'opérateur binaire $Addition$ dans notre table, aura le pattern \\ $P11 \rightarrow \ P12 \ \textrm{'}+\textrm{'} \ (P11 \ | \ P12)$
...
Il existe deux variantes d'opérateurs ternaires, la première étant le bloc conditionnel, "if ... then ... else", et l'autre ressemblant au "inline if", par exemple "... ? ... : ...". Pour décrire ces deux opérateurs, on dénombre les opérateurs par $op\_reprN$.
\begin{enumerate}
...
$next\_expression \ op\_repr+ \ curr\_expression$
