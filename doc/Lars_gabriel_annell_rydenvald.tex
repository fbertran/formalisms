...
Dans le cadre du logiciel \underline{\href{https://ardoises.ovh/overview}{Ardoises}}, ce travail a pour but d'introduire la possibilité aux utilisateurs de définir des formalismes opérateurs, de fournir un analyseur lexical capable de lire une expression basée sur ces opérateurs et d'en produire un arbre syntaxique abstrait, ainsi que de vérifier que les propriétés des opérateurs définis par les utilisateurs sont respectées
\section{Ardoises}
...
\mint{lua}{{ 3 % % % }}
\noindent ce qui n'est pas correct. On va donc opérer sur la liste qui nous est donnée par $LPeg$ pour créer la table juste.
\begin{minted}{lua}
