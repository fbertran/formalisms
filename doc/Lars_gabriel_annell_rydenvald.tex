...
\noindent Avec cette librairie, on a donc la possibilité de créer des modèles de base, puis de les spécialiser en définissant plus précisément les propriétés des modèles qui héritent du modèle de base. De plus, grâce au formalismes \lstinline|record| et \lstinline|collection| de la librairie \lstinline|Ardoises/Formalismes| on peut imposer des propriétés que les modèles plus spécifiques devront satisfaire.
...
(((((sum(30-20))))))
\end{minted}
LPeg finit en 2.233 secondes, tandis que LulPeg prend 210.791 secondes (3 minutes et 30 secondes). Ces résultats sont des moyennes calculées sur 1'000 exécutions, et chronométrées avec la commande \lstinline|time|, sur Ubuntu 16.04 et un ordinateur équipé d'un processeur Intel(R) Core(TM) i5-6200U CPU @ 2.30GHz. \\
...
\noindent Si on suppose que notre opérateur $addition$ est associatif à gauche, ce résultat est évidemment faux. La table au-dessus correspond à l'arbre syntaxique 
\begin{center}
	\begin{figure}[h]
		\centering
	...
	\end{figure}
\end{center}
Pour avoir une production correcte, il nous suffit d'opérer de manière récursive sur la table produite, on réarrange donc simplement les nœuds en traversant la table pour obtenir
\begin{center}
	\begin{figure}[h]
		\centering
...
\end{enumerate}
Le code décrivant cette transformation se trouve sur mon \underline{\href{http://dinosaur.compilertools.net/}{github}}
