...
\noindent Avec cette librairie, on a donc la possibilité de créer des modèles de base, puis de les spécialiser en définissant plus précisément les propriétés des modèles qui héritent du modèle de base. De même, grâce au formalisme \lstinline|record| de la librairie \lstinline|Ardoises/Formalismes| on peut imposer des propriétés que les modèles plus spécifiques devront satisfaire.
...
La modularité des définitions pour les expressions reste aussi importante; Le modèle d'expression le plus abstrait définit donc simplement qu'une expression est composée de
\begin{itemize}
...
\end{minted}
\noindent Cette représentation nous donnerait l'arbre syntaxique suivant
\begin{center}
	\begin{figure}[h]
