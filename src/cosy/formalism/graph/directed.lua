...
return function (Layer, directed, ref)
  ...
  directed [meta].edge_type [meta] [record] .source = { value_container = ref.vertices }
  directed [meta].edge_type [meta] [record] .target = { value_container = ref.vertices }

  local edge = Layer.reference (directed [meta].edge_type)

  directed [meta].edge_type.arrows .source = { vertex = edge.source }
  directed [meta].edge_type.arrows .target = { vertex = edge.target }
